...
// Combobox for picking a client company on the new-project form, with an inline
// "add a new company" path. It renders the hidden/visible inputs the
// createProject action reads: `clientCompanyId` when an existing company is
// picked, or `newCompany*` fields when a new one is being added.

type PickerCompany = Pick<
  ClientCompany,
  "id" | "name" | "contactName" | "contactEmail" | "designation"
>;

export default function ClientCompanyPicker({ companies }: { companies: PickerCompany[] }) {
        ...
        <div className="flex items-start justify-between gap-3">
          <div>
            ...
            <div className="mt-1 text-xs text-slate-400">
              This contact will be invited as the project&apos;s primary client contact.
            </div>
          </div>
          <button
            type="button"
    ...
    return (
      <div className="space-y-3 rounded-lg border border-slate-300 p-3 dark:border-slate-700">
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium text-slate-700 dark:text-slate-200">New client company</span>
          <button
            type="button"
            onClick={reset}
        ...
        <p className="text-xs text-slate-400">
          Saved to the shared Client Companies directory — next time you can just search for it.
        </p>
      </div>
    );
  }
